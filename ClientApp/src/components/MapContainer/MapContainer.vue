...
export default class MapContainer extends Vue {
  ...
  updateSource(geoJson: object): void {
    ...
    if (boatStyleTemp instanceof Style) {
      boatStyleTemp.getImage().setRotation(Math.PI / 180 * this.$store.state.positionData[this.$store.state.positionData.length - 1]?.heading)
    }
  }

  created() {
    this.$store.subscribe((mutation, state) => {
      const lineStringCoordinates = state.positionData.map((record: PositionRecord) => {
        return fromLonLat([record.lon, record.lat])
      })
      const boatCoordinate = fromLonLat([state.positionData[state.positionData.length - 1].lon, state.positionData[state.positionData.length - 1].lat])
